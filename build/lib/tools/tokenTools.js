...
const httpErrorCreator = createHttpError;
...
const createAccessToken = (payload) => new Promise(function (res, rej) {
    jwt.sign(payload, process.env.JWT_SECRET, { expiresIn: "1d" }, (err, token) => {
        if (err)
            rej(err); //if error reject
        else
            res(token); //else resolve promise
    });
});
export const verifyAccessToken = (accessToken) => new Promise((res, rej) => //verify access token
 jwt.verify(accessToken, process.env.JWT_SECRET, (err, originalPayload) => {
    if (err)
        res({ result: "fail" }); //if error resolve promise with result fail
    else
        res(originalPayload); //else resolve promise with original payload
}));
const createRefreshToken = (payload) => new Promise((res, rej) => {
...
const verifyRefreshToken = (refreshToken) => new Promise((res, rej) => //verify refresh token
